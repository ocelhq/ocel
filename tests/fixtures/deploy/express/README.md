...
An Express 5 app that declares no resources at all. It reads `GREETING` and `SECRET_TOKEN`
straight off `process.env`, so `ocel deploy` has nothing to provision and the journey
suites can ask whether the framework runs on a target at all.
