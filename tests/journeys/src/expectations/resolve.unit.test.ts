...
describe("resolve", () => {
  it("lists a test under every gap that names it", () => {
    const out = listed(
      [
        gap("one", [{ on: ["dev"], cells: ["sdk/node/web"], tests: [UP_TITLE] }], 1),
        gap("two", [{ on: ["dev"], cells: ["sdk/node/web"], tests: [UP_TITLE] }]),
      ],
      "dev",
    );
    assert.deepEqual(out["sdk/node/web"]?.[UP_TITLE], [
  ...
  it("lists a test once under a gap whose blocks overlap", () => {
    const out = listed(
      [
        gap("one", [
          { on: ["dev"], cells: ["sdk/node/web"], tests: [UP_TITLE] },
          { on: ["dev"], tests: [UP_TITLE] },
        ]),
      ],
      "dev",
    );
    assert.equal(out["sdk/node/web"]?.[UP_TITLE]?.length, 1);
    assert.equal(out["deploy/node/web"]?.[UP_TITLE]?.length, 1);
  });
  ...
  it("expands a set of rows, minus the titles it excepts", () => {
    ...
    const titles = Object.keys(out["sdk/node-container/web"] ?? {});
  ...
  it("refuses a block that names a cell whose plan has none of the tests", () => {
    assert.throws(
      ...
      /one on aws lists deploy\/node\/web/,
    );
  });
  ...
  it("skips a block listed for another environment", () => {
    ...
    assert.ok(listed(gaps, "vps")["sdk/node/web"]?.[UP_TITLE]);
  });
