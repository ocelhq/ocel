...
export default defineConfig({
  ...
  test: {
    ...
    // local-dev Bun harness's dev-only endpoints and its mounting of
    // @repo/api's handlers (including resolveResources), which aren't
    // exercised from @repo/api's own tests.
    ...
    env: {
      ...
      // so `pnpm test` works standalone (see packages/api/vitest.config.ts).
      ...
      // ocelhq-amu epic). Locally there's only one Postgres instance, so
      // this points at the same one as DATABASE_URL/TEST_DATABASE_URL but
      // authenticated as its superuser.
