...
function rememberSidebar(name: string, value: string) {
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      rememberSidebar(SIDEBAR_STATE_COOKIE, String(openState));
    },
    [setOpenProp, open],
  );

  const [width, _setWidth] = React.useState(() => sidebarWidth(defaultWidth));
  const setWidth = React.useCallback((value: number) => {
    const next = sidebarWidth(value);
    _setWidth(next);
    rememberSidebar(SIDEBAR_WIDTH_COOKIE, String(next));
  }, []);
    ...
    >
      ...
      >
        <div
          ...
          className="flex size-full flex-col bg-sidebar group-data-[variant=floating]:rounded-none group-data-[variant=floating]:shadow-float group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border"
        >
          {children}
        </div>
      </div>
  ...
  return (
    <div
      ...
      onPointerDown={(event) => {
        ...
        if (event.button !== 0 || !container || !wrapper) {
          return;
        }
        event.preventDefault();
      ...
      onKeyDown={(event) => {
        const side = event.currentTarget.closest<HTMLElement>('[data-slot="sidebar-container"]')
          ?.dataset.side;
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-none md:peer-data-[variant=inset]:shadow-float md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
        className,
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button relative flex w-full items-center gap-2 overflow-hidden rounded-none px-4 py-2 text-left text-sm leading-5 ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-foreground/5 hover:text-sidebar-foreground focus-visible:ring-2 active:bg-sidebar-foreground/5 active:text-sidebar-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-foreground/5 data-open:hover:text-sidebar-foreground data-active:bg-sidebar-foreground/5 data-active:font-medium data-active:text-sidebar-primary data-active:hover:bg-sidebar-foreground/5 data-active:hover:text-sidebar-primary data-active:active:text-sidebar-primary data-active:before:absolute data-active:before:inset-y-0 data-active:before:left-0 data-active:before:w-0.5 data-active:before:bg-sidebar-primary [&_svg]:size-4.5 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
        default: "hover:bg-sidebar-foreground/5 hover:text-sidebar-foreground",
        outline:
          "bg-background shadow-[0_0_0_1px_var(--sidebar-border)] hover:bg-sidebar-foreground/5 hover:text-sidebar-foreground hover:shadow-[0_0_0_1px_var(--sidebar-accent)]",
      },
      size: {
        default: "h-9",
        sm: "h-8",
