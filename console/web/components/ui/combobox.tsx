...
function trackScrollFade(node: HTMLElement | null) {
  if (!node) {
    return;
  }
  const update = () => {
    const above = Math.max(node.scrollTop, 0);
    const below = Math.max(node.scrollHeight - node.clientHeight - node.scrollTop, 0);
    node.style.setProperty("--fade-top", `${Math.min(above, scrollFade)}px`);
    node.style.setProperty("--fade-bottom", `${Math.min(below, scrollFade)}px`);
  };
  update();
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        "no-scrollbar max-h-[min(calc(--spacing(72)---spacing(9)),calc(var(--available-height)---spacing(9)))] scroll-py-1 overflow-y-auto overscroll-contain [mask-image:linear-gradient(to_bottom,transparent,black_var(--fade-top,0px),black_calc(100%_-_var(--fade-bottom,0px)),transparent)] data-empty:p-0",
        className,
      )}
      {...props}
      ref={trackScrollFade}
