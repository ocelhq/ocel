...
ensureConfig(slug);
console.error(`[ocel-e2e-node] removing preview ${ref} from project ${slug}`);
...
if (res.error || res.signal || res.status !== 0) {
  ...
  console.error(
    `[ocel-e2e-node] TEARDOWN FAILED for preview ${ref} of project ${slug}: ${why}\n` +
      `[ocel-e2e-node] its Lambdas and stacks are still live; remove them by running ` +
      `\`ocel preview rm --ref ${ref} --yes\` from a directory whose ocel.config.ts ` +
      `declares slug: "${slug}", or take the whole project with ` +
      `\`node scripts/e2e-node/project-teardown.mjs ${slug}\``,
  );
  process.exit(1);
}

console.error(`[ocel-e2e-node] preview ${ref} removed`);
