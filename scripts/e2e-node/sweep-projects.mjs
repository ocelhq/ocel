  ...
  for (;;) {
    try {
      ...
      if (Date.now() >= deadline) {
        console.error(
          `[ocel-e2e-node] could not list ${PREVIEW_ROOT_STACK_PARAM_PREFIX} at all within ` +
      ...
      console.error(
        `[ocel-e2e-node] could not list ${PREVIEW_ROOT_STACK_PARAM_PREFIX} (${err.message}); will retry`,
      );
...
if (stranded.length === 0) {
  console.error(`[ocel-e2e-node] no stranded node-suite projects; ${keep} is the only one`);
  process.exit(0);
}

console.error(`[ocel-e2e-node] ${stranded.length} stranded node-suite project(s): ${stranded.join(", ")}`);

const failed = stranded.filter((slug) => !destroyProject(slug));
if (failed.length > 0) {
  console.error(
    `[ocel-e2e-node] could not reclaim ${failed.join(", ")} — their preview footprint keeps billing`,
  );
  process.exit(1);
}

console.error(`[ocel-e2e-node] reclaimed ${stranded.length} stranded project(s)`);
