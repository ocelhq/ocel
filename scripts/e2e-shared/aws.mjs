// The `aws` CLI calls the deployment-e2e harnesses' assertion scripts share
// (scripts/e2e-next/assert-{bytecode,embed}.mjs and their e2e-node
// counterparts).
//
// Deliberately not in lib.mjs: that module is this package's pure logic, unit
// tested transitively through each harness's own lib.test.mjs (which
// re-exports it). Nothing here can be, because every function shells out to a
// real account — so the two live apart rather than leaving lib.mjs half
...
// spelling a suite's deploys can ever produce a key under.
...
// way a harness's own logs.mjs (or, for e2e-node, its deploy log) finds its log
// groups: by the ocel tags every Ocel function carries (cloud/aws/deploy/
// function.go). Both `ocel:project` and `ocel:app` are filtered on, unlike a
// project-only filter, because the keys these scripts compose are one
// function's — a project with more than one app would otherwise leave which
// function ambiguous.
