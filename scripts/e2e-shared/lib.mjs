...
/**
 * The file a harness's deploy script persists this app's identity to. The one
 * definition every harness's `lib.mjs` re-exports via `export *` rather than
 * shadowing with its own — scripts/e2e-next and scripts/e2e-node used to each
 * declare this (and BUILD_LOG_FILE, DEPLOY_RESULT_FILE below) a second time,
 * silently making the shared one unreachable.
 */
 ...
 * Built with `join` rather than a literal `.ocel/deploy-result.json` so it
 * reads correctly on Windows too — the two are POSIX-equal, but a literal
 * forward-slash path silently does not.
 */
...
export function renderOcelConfig({ slug, previewDomain, apps }) {
  const lines = [
    ...
    ``,
    `// Generated by an ocel e2e harness (scripts/e2e-next or scripts/e2e-node). Do not edit.`,
...
const MAX_WORKER_NAME_LEN = 63;

/**
 * sanitizeWorkerName mirrors sanitizeWorkerName in
 * cloud/aws/deploy/edgeworker.go byte-for-byte: lowercase, every character
 * outside [a-z0-9] collapsed into a single '-', leading/trailing hyphens
 * trimmed, clamped to the platform limit.
 *
 * Both appAssetPrefix and bytecodeAppNamespace apply this to `app` before
 * composing a key, the same as their Go counterparts do — an app is never
 * assumed to already be a valid worker name here, because a harness that
 * passed one that was not would otherwise derive a namespace Go never
 * writes to, list zero objects, and fail with a message that never
 * mentions why. Every existing caller in this repo already passes an
 * already-valid, fixed app name ("app"), which this leaves untouched
 * (sanitizeWorkerName(s) === s for such names), so applying it here changes
 * no key any current harness composes.
 */
export function sanitizeWorkerName(s) {
  let out = "";
  ...
  out = trimHyphens(out);
  if (out.length > MAX_WORKER_NAME_LEN) {
    out = trimHyphens(out.slice(0, MAX_WORKER_NAME_LEN));
  }
  return out || "ocel-worker";
}
 ...
 * cloud/aws/deploy/prerender.go: <env>/<slug>/<sanitizeWorkerName(app)>/<buildId>.
 */
export function appAssetPrefix({ environment, slug, app, buildId }) {
  return [envSegment(environment), slug, sanitizeWorkerName(app), buildId].join("/");
}
 ...
 * bytecode/<env>/<slug>/<sanitizeWorkerName(app)>.
 *
...
export function bytecodeAppNamespace({ environment, slug, app }) {
  return ["bytecode", envSegment(environment), slug, sanitizeWorkerName(app)].join("/");
}
