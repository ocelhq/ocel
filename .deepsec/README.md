...
Currently configured project: `ocelhq` (target: `..`).
...
original init command. Checkpoints in `data/ocelhq/setup/setup-state.json`
...
`--project-id ocelhq` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/ocelhq/`.
