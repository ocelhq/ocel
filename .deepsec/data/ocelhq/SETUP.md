# Agent setup for `ocelhq`

This is a deepsec scanning workspace. Project `ocelhq` was just registered
(target: `..`). Setup is incomplete — `data/ocelhq/INFO.md`
...
2. **Fill in `data/ocelhq/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id ocelhq
pnpm deepsec process --project-id ocelhq
```
