// The two attribute forms a tag record is made of. DynamoDB's JSON body spells
// every value as a single-key object naming its type, and numbers travel as
// strings — so this is at once what the AWS SDK's UpdateItemCommand accepts and
// what a raw signed request puts on the wire. Declared here rather than imported
// so the package stays dependency-free, which is what lets the Lambda (SDK) and
// the worker (signed HTTP) build the identical update through unlike transports.
export type TagAttribute = { S: string } | { N: string };

// A DynamoDB UpdateItem body, narrowed to exactly the fields a tag write sets.
