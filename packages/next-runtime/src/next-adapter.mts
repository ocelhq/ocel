  ...
  async onBuildComplete(args) {
    ...
    const routingManifest = {
      ...
      dispatch: Object.fromEntries([
        ...
        //
        // tags and allowQuery ride along because the edge cache cannot recover
        // them at request time: Next never emits x-next-cache-tags on an HTTP
        // response (it is internal server<->cacheHandler plumbing), so a worker
        // caching this route would have nothing to stamp Cache-Tag with and
        // every purge would silently no-op.
        ...outputs.prerenders.map((p) => {
          ...
          return [
            p.pathname,
            {
              ...
              ...(tags.length > 0 && { tags }),
              // Absent and empty mean different things — [] drops every query
              // param from the cache key, undefined states no rule at all.
