...
A Next.js App Router app that declares nothing. There is no `ocel/` directory, no SDK call
and no resource: the config carries a slug and one app, and the app answers `/health` and
serves `ocel.svg` out of `public/`.
