...
func TestABindingOnlyTheLiveDirectoryHoldsIsReadFromItsFile(t *testing.T) {
	dir := t.TempDir()
	record := `{"name":"main","postgres":{"host":"h","port":5432,"database":"d","username":"u","password":"p"}}`
	if err := os.WriteFile(filepath.Join(dir, "OCEL_RESOURCE_POSTGRES_main"), []byte(record), 0o600); err != nil {
		t.Fatal(err)
	}
