...
func link(name string, typ linksv1.LinkType) (*linksv1.Link, error) {
	...
	raw := os.Getenv(key)
	if raw == "" {
		//nolint:staticcheck // the wording is the one every ocel sdk prints, sentence and all
	...
	if err := protojson.Unmarshal([]byte(raw), delivered); err != nil {
		return nil, fmt.Errorf("%s does not carry a link record, so this app cannot read it as a %s: %w", key, kindOf(typ), err)
	}
