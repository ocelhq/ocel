...
export async function loadUserApp(entrypoint: string): Promise<Loaded> {
  ...
  const result = await Promise.race([
    serverPromise,
    importPromise.then((r) => (servable(r.value) ? r : serverPromise)),
  ]);
...
function servable(exported: unknown): boolean {
  try {
    resolveHandler(exported);
    return true;
  } catch {
    return false;
  }
...
export function resolveHandler(exported: any): Resolved {
...
export function invokeFor(resolved: Resolved): Invoke {
