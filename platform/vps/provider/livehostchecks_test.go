...
func about(t *testing.T, checks []provider.HostCheck, subject string) provider.HostCheck {
	t.Helper()
...
func TestLiveTheHostCheckVerdictsReadOffABootstrappedBoxAndGateNothing(t *testing.T) {
	...
	if reach.Verdict != provider.HostPass {
	...
	if admin.Verdict != provider.HostPass {
...
func TestLiveTheAdminPortBoundInsideTheProxyFailsTheHostCheckVerdict(t *testing.T) {
	...
	if before.Verdict != provider.HostPass {
	...
	if after.Verdict != provider.HostFail {
