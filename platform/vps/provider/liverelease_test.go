...
func provisioned(t *testing.T, p *vps.Provider, tag string) release {
	t.Helper()
	started, err := p.ProvisionContainers(context.Background(), liveSpec(t, tag), nil)
	if err != nil {
...
func inFlightAgainst(t *testing.T, vm machine, serving release, want int) {
	t.Helper()
...
func TestLiveAReleaseServesThroughTheProxyAndNothingElseOnTheBoxCanReachIt(t *testing.T) {
	...
	if ref := vm.inspects(t, "container", one.physical, host.LabelSelector(host.LabelRef)); ref != fixtureAt("one") {
		t.Errorf("the container has %q under %s, want the image ref it runs", ref, host.LabelRef)
	}
	...
	if mode := strings.TrimSpace(vm.inside(t, "stat -c %a "+quote(caddy.AdminSocket))); mode != "600" {
		t.Errorf("%s is %q, want 600: the socket's permissions are the whole of its access control", caddy.AdminSocket, mode)
	}
}

func TestLiveARedeployUnderContinuousLoadDropsNothingAndDrainsWhenTheSlowRequestReturns(t *testing.T) {
	vm, p := onABoxServingContainers(t)
	...
	go func() {
		defer group.Done()
		slow = vm.peers(t, "curl -sS -m 30 -w ' %{http_code}' 'http://"+caddy.Container+"/hold?s=8'")
	}()
	go func() {
		defer group.Done()
		for {
...
func TestLiveARequestOutstandingPastTheDrainWindowGetsFiveOhTwo(t *testing.T) {
	...
	go func() {
		defer group.Done()
		slow = vm.peers(t, "curl -sS -m 60 -o /dev/null -w '%{http_code}' 'http://"+caddy.Container+"/hold?s=25'")
	}()
	inFlightAgainst(t, vm, one, 1)
	...
	if !strings.Contains(slow, "502") {
		t.Errorf("a request outstanding past the drain window answered %q, want 502: it dies with its backend rather than being cancelled by the proxy", strings.TrimSpace(slow))
	}
	if vm.running(t, one.physical) {
...
func TestLiveAReleaseThatCannotPassItsGateLeavesThePreviousOneServing(t *testing.T) {
	...
	for _, tag := range []string{"sick", "hung", "crasher"} {
		...
		if served := servedBy(t, vm, "/"); served != "one" {
			t.Errorf("after the %s release failed the proxy serves %q, want the previous release still in place", tag, served)
		}
		...
		if id := vm.inspects(t, "container", broken.physical, "{{.Id}}"); id != "" {
			t.Errorf("the %s release left its container in place as %s", tag, id)
		}
		if config := strings.TrimSpace(vm.ssh(t, "sudo cat "+host.ProxyConfig)); strings.Contains(config, broken.physical) {
...
func TestLiveAHijackedConnectionDrainsWithItsRetireeRatherThanBeingCutAtCutover(t *testing.T) {
	...
	if lived := seconds(t, upgraded); lived < window.Seconds() {
		t.Errorf("the hijacked connection lived %.1fs, shorter than the %s drain window, so the reload at cutover cut it rather than leaving it the window a stream to the retiree is promised", lived, window)
	}
	...
	if took < window-time.Second {
		t.Errorf("the release returned in %s inside a %s window with an sse stream still open, so the stream no longer keeps the retired upstream open and the statement that an sse app drains at its ceiling is wrong", took, window)
	}
	if warned.at("still had") < 0 {
	...
	if vm.running(t, one.physical) {
		t.Error("the retired container survived a deploy that kept its connections open past the window")
	}
	if served := servedBy(t, vm, "/"); served != "two" {
