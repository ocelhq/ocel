...
func TestABootstrapIsRefusedBeforeItWritesAnythingAsRoot(t *testing.T) {
	t.Parallel()
	...
	if _, err := gated.Plan(ctx, req); err == nil {
		t.Error("Plan() = nil, want the refusal that names what the login lacks before a plan is drawn over a host it cannot write to")
	}
	...
	if inner.planned+inner.applied+inner.removed != 0 {
		t.Errorf("the host was reached %+v times through a login that cannot write as root, want nothing attempted", *inner)
	}
}
