...
const (
	answersTimeout = 5 * time.Second
	answersCap     = 1 << 20
)

func answers(argv []string, errs io.Writer) int {
	if len(argv) != 2 || argv[0] == "" || !strings.HasPrefix(argv[1], "/") {
		return usage(errs)
	}
	socket, path := argv[0], argv[1]
	dialer := &net.Dialer{}
	client := &http.Client{Timeout: answersTimeout, Transport: &http.Transport{
	...
	body, err := io.ReadAll(io.LimitReader(said.Body, answersCap))
	...
	case len(trimmed) == 0 || string(trimmed) == "null":
		fmt.Fprintf(errs, "%s: %s serves nothing at %s\n", switchboard.Name, socket, path)
	default:
		return 0
	}
	return exitNotServingYet
