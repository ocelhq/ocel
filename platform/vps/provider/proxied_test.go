...
func TestABoxRefusesAProxiedBindingItServesNothingFor(t *testing.T) {
	...
	var unserved *providerserver.UnservedProxiedBindingError
	...
	for _, want := range []string{"queue--jobs", "queue", string(vps.Vendor)} {
		if !strings.Contains(unserved.Error(), want) {
			t.Errorf("the refusal reads %q and never names %q", unserved.Error(), want)
		}
	}
	if strings.Contains(strings.ToLower(unserved.Error()), "aws") {
		t.Errorf("a box's refusal reads %q and names another vendor's provider", unserved.Error())
	}
}
