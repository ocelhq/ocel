...
func TestAPreviewWildcardIsAskedAboutUnderTheNameThatResolves(t *testing.T) {
	...
	if checks[0].Verdict != provider.HostPass {
...
func adminCheck(t *testing.T, checks []provider.HostCheck) provider.HostCheck {
	t.Helper()
	if len(checks) == 0 {
...
func boardCheck(t *testing.T, checks []provider.HostCheck) provider.HostCheck {
	t.Helper()
	for _, check := range checks {
