...
func TestRenderAppBundle(t *testing.T) {
	...
	t.Run("references only the owners of its own live values", func(t *testing.T) {
		...
		role := appExecutionRole(cfg, "web", nil, nil, bundle)
		...
		other := appExecutionRole(cfg, "admin", nil, nil, appBundle{Live: []byte(`{"slug":"shop"}`)})
...
func TestAppExecutionRoleLiveValues(t *testing.T) {
	t.Run("takes the table only for an app with live values", func(t *testing.T) {
		...
		cfg := liveConfig()
		cfg.Slug = "shop"
		withLive := appExecutionRole(cfg, "web", nil, nil, appBundle{Live: []byte(`{"slug":"shop"}`)})
		...
		withoutLive := appExecutionRole(cfg, "admin", nil, nil, appBundle{})
