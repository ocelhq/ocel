...
func TestAppExecutionRole(t *testing.T) {
	...
	t.Run("carries the substrate's vars key", func(t *testing.T) {
		...
		role := appExecutionRole(Config{VarsKeyARN: productionVarsKeyARN}, "web", caches, nil, appBundle{}, nil)
		...
		preview := appExecutionRole(Config{VarsKeyARN: previewVarsKeyARN}, "api", caches, nil, appBundle{}, nil)
