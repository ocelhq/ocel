...
func TestRoute53RereadsTheZonesWhenNoneItRememberedOwnsTheName(t *testing.T) {
	...
	if _, err := writer.EnsureRecords(t.Context(), []edge.Record{first}, nil); err != nil {
	...
	if _, err := writer.EnsureRecords(t.Context(), []edge.Record{later}, nil); err != nil {
		t.Fatalf("EnsureRecords into a zone made since the first listing: %v", err)
	}
	if api.lists <= listed {
