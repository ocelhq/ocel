...
func TestAContainerAppIsRefusedBehindTheFunctionInvokingEdgeAlone(t *testing.T) {
	t.Parallel()
	...
	if err := refuseContainersBehindFunctionEdge(providerkit.DeployPreflight{Edge: cloudfront.Kind, Plan: providerkit.DeployPlan{Apps: apps}}); err != nil {
		t.Fatalf("preflight behind %s = %v, want it to pass: that edge reaches an origin by URL", cloudfront.Kind, err)
	}
