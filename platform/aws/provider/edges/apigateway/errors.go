...
func createAPIError(name string, err error) error {
	if atAPICeiling(err) {
		return fmt.Errorf("create REST API %q: this account is at its API Gateway limit in this region, so there is no room for another one. Every Ocel deployment the %q edge fronts gets its own regional REST API, and the account-wide ceiling is the \"Regional APIs per Region per account\" quota for Amazon API Gateway. Open the Service Quotas console in this region, find Amazon API Gateway, request an increase on that quota, and re-run the deploy once AWS grants it. If you would rather not raise it, delete the REST APIs left behind by projects you no longer serve - `ocel destroy production` on a retired project removes its API - and the next deploy will fit: %w", name, Kind, err)
	}
	if throttled(err) {
