...
func TestPromotionHistory(t *testing.T) {
	...
	t.Run("the newest promotion is the active one", func(t *testing.T) {
		...
		if err := stackOn(p, state).Promote(t.Context(), promotion, "", edge.DiscardProgress()); err != nil {
	...
	t.Run("pruning keeps the window and pins the active promotion", func(t *testing.T) {
		...
		for _, id := range []string{"p1", "p2", "p3"} {
			if err := stackOn(p, state).Promote(t.Context(), edge.Promotion{PromotionID: id, Ts: 1, Builds: map[string]string{"web": id}}, "", edge.DiscardProgress()); err != nil {
...
func TestStorePointer(t *testing.T) {
	...
	t.Run("promote, history and prune send the pointer only when there is one", func(t *testing.T) {
		...
		if err := stackOn(p, state).Promote(ctx, edge.Promotion{PromotionID: "p1", Ts: 1, Builds: map[string]string{"web": "b1"}}, "pr-42", edge.DiscardProgress()); err != nil {
		...
		if err := stackOn(p, state).Promote(ctx, edge.Promotion{PromotionID: "p2", Ts: 2, Builds: map[string]string{"web": "b2"}}, "", edge.DiscardProgress()); err != nil {
