...
describe("cacheKey", () => {
  ...
  it("scopes the key by app so two apps of one project never collide", () => {
    ...
    expect(web).not.toEqual(admin);
  });

  it("scopes the key by project so two projects never collide", () => {
    const url = new URL("https://app.example/blog");
  ...
  it("gives an RSC request a different key than the HTML request", () => {
    const url = new URL("https://app.example/blog");
    const html = cacheKey(scope, "/blog", url, H(), "STATIC", []);
    const rsc = cacheKey(scope, "/blog", url, H({ RSC: "1" }), "STATIC", []);
  ...
  it("gives an intercepted segment prefetch a different colo key than the same URL without next-url", () => {
    ...
    const plain = cacheKey(scope, "/photo", url, H(headers), "PARTIALLY_STATIC", []);
...
describe("serveCached", () => {
  ...
  it("never serves a stored postponed shell back under a segment-scoped key", async () => {
    const clock = { ms: 0 };
    const deps = testDeps(clock);
    const t = target("poisoned-segment", { segment: true });
    await storeInColo(
      t,
      deps,
      new Response("shell", {
        headers: { "cache-control": "s-maxage=60", "x-nextjs-postponed": "1" },
      }),
    );

    const origin = async () => new Response(null, { status: 204 });
  ...
  it("still serves a stored segment payload under a segment-scoped key", async () => {
    const clock = { ms: 0 };
    const deps = testDeps(clock);
    const t = target("stored-segment", { segment: true });
    await storeInColo(
      t,
      deps,
      new Response("SEGMENT", {
        headers: { "cache-control": "s-maxage=60", "x-nextjs-postponed": "2" },
      }),
    );
    ...
    expect(res.headers.get("x-ocel-cache")).toBe("HIT");
