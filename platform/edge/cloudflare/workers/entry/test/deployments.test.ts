...
function makeRecord(over: Partial<DeploymentRecord> = {}): DeploymentRecord {
  return {
    ...
    isrPrefix: "prod/p1/web/deploy-1",
    createdAt: 1_000,
    ...over,
  };
}
...
function countingBinding(opts: {
  pointerDeploymentId: Record<string, string | undefined>;
  ...
  return {
    ...
    async pointerRecord(args: {
      ...
      const deploymentId = opts.pointerDeploymentId[`${args.app}/${args.pointer ?? ""}`];
      if (!deploymentId) {
        this.lastCarriedRecord = false;
        return { kind: "no-pointer" };
      }
      if (deploymentId === args.knownDeploymentId) {
        this.lastCarriedRecord = false;
        return { kind: "unchanged", deploymentId };
      }
      const record = opts.records[`${args.app}/${deploymentId}`];
      if (!record) {
        this.lastCarriedRecord = false;
        return { kind: "dangling", deploymentId };
      }
      this.lastCarriedRecord = true;
      return { kind: "record", deploymentId, record };
    },
  };
}
...
describe("resolveDeployment", () => {
  it("resolves and returns the active Deployment record", async () => {
    const binding = countingBinding({
      pointerDeploymentId: { "web/": "deploy-1" },
  ...
  it("returns not-found when no active pointer exists for the app", async () => {
    const binding = countingBinding({ pointerDeploymentId: {}, records: {} });
    const clock = { ms: 0 };
  ...
  it("re-reads the record when the deployment moves (promotion/rollback)", async () => {
    const pointerDeploymentId: Record<string, string> = { "web/": "deploy-1" };
    const binding = countingBinding({
      pointerDeploymentId,
      records: {
        "web/deploy-1": makeRecord(),
        "web/deploy-2": makeRecord({ deploymentId: "deploy-2" }),
      },
    });
    const clock = { ms: 0 };
    const d = deps(binding, clock);
    ...
    pointerDeploymentId["web/"] = "deploy-2";
    clock.ms = 5_001;
  ...
  it("returns unavailable when the pointer names a deployment with no record", async () => {
    const binding = countingBinding({
      pointerDeploymentId: { "web/": "deploy-1" },
      records: {},
    });
  ...
  it("keeps caches independent across apps", async () => {
    const binding = countingBinding({
      pointerDeploymentId: { "web/": "deploy-1", "admin/": "deploy-9" },
      records: {
        "web/deploy-1": makeRecord(),
        "admin/deploy-9": makeRecord({ app: "admin", deploymentId: "deploy-9" }),
      },
    });
    const clock = { ms: 0 };
  ...
  it("resolves a named preview pointer independently of the default", async () => {
    const previewRecord = makeRecord({ deploymentId: "preview-deploy" });
    const binding = countingBinding({
      pointerDeploymentId: {
        "web/": "deploy-1",
  ...
  it("keeps caches independent across projects sharing one binding", async () => {
    const records: Record<string, DeploymentRecord> = {
      acme: makeRecord({ isrPrefix: "prev/acme/web/deploy-1" }),
      globex: makeRecord({ isrPrefix: "prev/globex/web/deploy-1" }),
    };
  ...
  it("caches on the host, so one host reuses the entry within the TTL", async () => {
    ...
    const binding: DeploymentsBinding = {
      async pointerRecord() {
        calls++;
        return { kind: "record", deploymentId: "deploy-1", record: makeRecord() };
      },
    };
    const clock = { ms: 0 };
  ...
  it("evicts the oldest host once the cache is full", async () => {
    ...
    const binding: DeploymentsBinding = {
      async pointerRecord(args) {
        ...
        return { kind: "record", deploymentId: args.slug, record: makeRecord() };
      },
    };
    const clock = { ms: 0 };
