...
func PropertyShapeMessages(shapes []PropertyShape) []*deploymentsv1.PropertyShape {
	out := make([]*deploymentsv1.PropertyShape, 0, len(shapes))
	for _, s := range shapes {
		out = append(out, &deploymentsv1.PropertyShape{Name: s.Name, JsonType: s.JSONType, List: s.List})
	}
	return out
}
