...
func TestTheRootSchemaIsWrittenOnceAndReadBack(t *testing.T) {
	...
	if written, err := providerkit.RecordSchema(ctx, records); err != nil || written != 0 {
		t.Fatalf("RecordSchema() of an unwritten tree = %d, %v, want 0", written, err)
	}
	if err := providerkit.EnsureRecordSchema(ctx, records); err != nil {
		t.Fatal(err)
	}
	held, err := records.Read(ctx, providerkit.SchemaRecord())
	if err != nil {
		t.Fatal(err)
	}
	if err := providerkit.EnsureRecordSchema(ctx, records); err != nil {
	...
	again, err := records.Read(ctx, providerkit.SchemaRecord())
	if err != nil {
		t.Fatal(err)
	}
	...
	if written, err := providerkit.RecordSchema(ctx, records); err != nil || written != providerkit.RecordSchemaVersion {
...
func TestARecordTreeANewerOcelWroteIsRefused(t *testing.T) {
	...
	if _, err := records.Write(ctx, providerkit.Record{Name: providerkit.SchemaRecord(), Bytes: []byte(ahead)}); err != nil {
		t.Fatal(err)
	}

	err := providerkit.EnsureRecordSchema(ctx, records)
