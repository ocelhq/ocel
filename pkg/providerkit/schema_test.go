...
func TestTheRootSchemaIsWrittenOnceAndReadBack(t *testing.T) {
	...
	if written, err := providerkit.RecordSchema(ctx, records, edge.ClassProduction); err != nil || written != 0 {
		t.Fatalf("RecordSchema() of an unwritten tree = %d, %v, want 0", written, err)
	}
	if err := providerkit.EnsureRecordSchema(ctx, records, edge.ClassProduction); err != nil {
		t.Fatal(err)
	}
	held, err := records.Read(ctx, providerkit.SchemaRecord(edge.ClassProduction))
	if err != nil {
		t.Fatal(err)
	}
	if err := providerkit.EnsureRecordSchema(ctx, records, edge.ClassProduction); err != nil {
	...
	again, err := records.Read(ctx, providerkit.SchemaRecord(edge.ClassProduction))
	if err != nil {
		t.Fatal(err)
	}
	...
	if written, err := providerkit.RecordSchema(ctx, records, edge.ClassProduction); err != nil || written != providerkit.RecordSchemaVersion {
...
func TestARecordTreeAnOlderOcelWroteIsRefused(t *testing.T) {
	...
	if _, err := store.Write(ctx, records.Record{Name: providerkit.SchemaRecord(edge.ClassProduction), Bytes: []byte(behind)}); err != nil {
		t.Fatal(err)
	}

	err := providerkit.EnsureRecordSchema(ctx, store, edge.ClassProduction)
	var refused refusal.Refusal
	if !errors.As(err, &refused) || refused.Code != refusal.CodeNotReady {
		t.Fatalf("EnsureRecordSchema() over an older tree = %v, want it refused as not ready", err)
	}
	if !strings.Contains(refused.Message, behind) || !strings.Contains(refused.Message, strconv.Itoa(providerkit.RecordSchemaVersion)) {
		t.Errorf("refusal = %q, want it to name both the schema written and the schema this build reads", refused.Message)
	}
	held, err := store.Read(ctx, providerkit.SchemaRecord(edge.ClassProduction))
...
func TestARecordTreeANewerOcelWroteIsRefused(t *testing.T) {
	...
	if _, err := store.Write(ctx, records.Record{Name: providerkit.SchemaRecord(edge.ClassProduction), Bytes: []byte(ahead)}); err != nil {
		t.Fatal(err)
	}
