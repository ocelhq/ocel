...
func TestArtifactsRemovePrefixLeavesTheRest(t *testing.T) {
	...
	kept := providerkit.ArtifactRef{Bucket: "bucket", Key: "other/app.zip"}
	removed := providerkit.ArtifactRef{Bucket: "bucket", Key: "releases/r1/app.zip"}
	...
	if err := artifacts.RemovePrefix(ctx, "bucket/releases/", nil); err != nil {
