...
const FakeLinksStoreEnvVar = "OCEL_TEST_FAKE_LINKS_STORE"
...
func loadFakeLinkStore() (fakeLinkStore, error) {
	store := fakeLinkStore{}
	raw, err := os.ReadFile(os.Getenv(FakeLinksStoreEnvVar))
...
func saveFakeLinkStore(store fakeLinkStore) error {
	...
	return os.WriteFile(os.Getenv(FakeLinksStoreEnvVar), raw, 0o600)
}
