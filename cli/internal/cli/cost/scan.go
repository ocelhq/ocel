...
func scanManifest(ctx context.Context, deps cmddeps.Deps, cfg *projectconfig.Config, env *environmentv1.Environment, out io.Writer) (*contractv1.Manifest, []string, error) {
	...
	manifest, err := manifestbuilder.Build(cfg.Slug, cfg.Domains, scannedApps(cfg), string(providerkit.ComputeServerless), manifestwire.Declarations(cfg.Dir, resources), manifestwire.Bindings(cfg.BindingsFor(projectconfig.Tier(env.GetTier() == environmentv1.Tier_TIER_PREVIEW))), functions, nil)
	if err != nil {
		return nil, nil, err
	}
