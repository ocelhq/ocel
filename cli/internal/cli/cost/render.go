...
func render(stdout io.Writer, slug string, set *costv1.ResourceSet, estimates map[string]*costv1.Estimate, profile string) error {
	chosen := estimates[profile]
	fmt.Fprintf(stdout, "%s · %s profile · %s per month · rates %s\n\n", slug, profile, chosen.GetCurrency(), chosen.GetRatesVersion())
	...
	fmt.Fprintln(stdout)
	fmt.Fprintf(stdout, "Fixed %s + usage %s = %s per month on the %s profile\n", money(chosen.GetMonthlyFixed()), money(chosen.GetMonthlyUsage()), sum(chosen.GetMonthlyFixed(), chosen.GetMonthlyUsage()), profile)
	fmt.Fprintf(stdout, "Usage by profile: light %s · moderate %s · heavy %s\n",
		money(estimates[profileLight].GetMonthlyUsage()), money(estimates[profileModerate].GetMonthlyUsage()), money(estimates[profileHeavy].GetMonthlyUsage()))
	...
	for typ, count := range coverage.GetUnsupportedTypes() {
