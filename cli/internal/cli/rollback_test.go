...
func TestRunRollback(t *testing.T) {
	...
	t.Run("--to rolls back to the named promotion", func(t *testing.T) {
		...
		var stdout, stderr bytes.Buffer
		if err := runRollback(context.Background(), deps, root, rollbackOptions{to: "promo-2"}, &stdout, &stderr); err != nil {
			t.Fatalf("runRollback err = %v; stdout=%s stderr=%s", err, stdout.String(), stderr.String())
		}
	...
	t.Run("--tag rolls back to the tagged promotion and echoes the tag", func(t *testing.T) {
		...
		var stdout, stderr bytes.Buffer
		if err := runRollback(context.Background(), deps, root, rollbackOptions{tag: "v1.0.0"}, &stdout, &stderr); err != nil {
			t.Fatalf("runRollback err = %v; stdout=%s stderr=%s", err, stdout.String(), stderr.String())
		}
	...
	t.Run("--to and --tag are mutually exclusive", func(t *testing.T) {
		...
		var stdout, stderr bytes.Buffer
		err := runRollback(context.Background(), deps, root, rollbackOptions{to: "promo-1", tag: "v1.0.0"}, &stdout, &stderr)
		if err == nil {
	...
	t.Run("an invalid tag errors", func(t *testing.T) {
		...
		var stdout, stderr bytes.Buffer
		err := runRollback(context.Background(), deps, root, rollbackOptions{tag: "feature/x"}, &stdout, &stderr)
		if err == nil {
	...
	t.Run("an unknown --to names the promotion it could not find", func(t *testing.T) {
		...
		var stdout, stderr bytes.Buffer
		err := runRollback(context.Background(), deps, root, rollbackOptions{to: "no-such-promotion"}, &stdout, &stderr)
		if err == nil {
	...
	t.Run("it refuses on preview infrastructure", func(t *testing.T) {
		...
		var stdout, stderr bytes.Buffer
		err := runRollback(context.Background(), deps, root, rollbackOptions{}, &stdout, &stderr)
		if err == nil {
