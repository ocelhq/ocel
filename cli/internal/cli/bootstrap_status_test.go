...
func bootstrap(tier environmentv1.Tier, stacks ...*contractv1.BootstrapStack) *contractv1.BootstrapStatus {
...
func TestRenderBootstrapStatus(t *testing.T) {
	t.Run("it reports every column for every stack", func(t *testing.T) {
		status := bootstrap(environmentv1.Tier_TIER_PRODUCTION,
	...
	t.Run("an absent bootstrap says so and nothing more", func(t *testing.T) {
		var out bytes.Buffer
		renderBootstrapStatus(&out, &contractv1.BootstrapStatus{Tier: environmentv1.Tier_TIER_PREVIEW, RequiredSchema: 1})
	...
	t.Run("it names the side that has to move", func(t *testing.T) {
		behind := bootstrap(environmentv1.Tier_TIER_PRODUCTION,
			&contractv1.BootstrapStack{Name: "ocel-bootstrap", Present: true, DigestCurrent: true})
		behind.Schema, behind.RequiredSchema = 1, 2
		ahead := bootstrap(environmentv1.Tier_TIER_PRODUCTION,
			&contractv1.BootstrapStack{Name: "ocel-bootstrap", Present: true, DigestCurrent: true})
		ahead.Schema, ahead.RequiredSchema = 3, 2

		if got := bootstrapProblem(behind); !strings.Contains(got, "ocel bootstrap") {
			t.Errorf("problem = %q, want it to point at bootstrap", got)
		}
		if got := bootstrapProblem(ahead); !strings.Contains(got, "upgrade the Ocel CLI") {
...
func TestBootstrapCheck(t *testing.T) {
	current := bootstrap(environmentv1.Tier_TIER_PRODUCTION,
		&contractv1.BootstrapStack{Name: "ocel-bootstrap", Present: true, Schema: 1, DigestCurrent: true})
	stale := bootstrap(environmentv1.Tier_TIER_PREVIEW,
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := bootstrapCheck(tc.statuses)
...
func TestRunBootstrapStatus(t *testing.T) {
	...
	t.Run("--check fails on stale content", func(t *testing.T) {
		root, _ := setUpDeployFixture(t)
		t.Setenv(fakeBootstrapEnvVar, "stale")
		d := defaultDeps()
		setLoggedIn(&d)
		stubAppFunctions(&d, nil)
...
func TestRunBootstrapDowngrade(t *testing.T) {
	t.Run("it warns and takes a confirmation before writing older content", func(t *testing.T) {
		root, _ := setUpDeployFixture(t)
		t.Setenv(fakeBootstrapEnvVar, "downgrade")
		d := defaultDeps()
		setLoggedIn(&d)
		stubAppFunctions(&d, nil)
