...
func TestResolvedEnv(t *testing.T) {
	...
	t.Run("the dotfile outranks every other source but a resource", func(t *testing.T) {
		...
		got := toMap(mergeEnv(base, projectEnv, live, dotfile, resources, "", "", envgate.Scope{}))
	...
	t.Run("live values are delivered at startup", func(t *testing.T) {
		...
		got := resolvedEnv(projectEnv, live, nil, resources, "", "", envgate.Scope{})
	...
	t.Run("the app folder is always stated", func(t *testing.T) {
		...
		bound := resolvedEnv(nil, nil, nil, nil, "", "/web", envgate.Scope{})
		...
		unbound := resolvedEnv(nil, nil, nil, nil, "", "", envgate.Scope{})
		...
		stale := toMap(mergeEnv([]string{constants.AppFolderEnvName + "=/stale"}, nil, nil, nil, nil, "", "", envgate.Scope{}))
...
func TestDevGivesEveryAppItsURL(t *testing.T) {
	...
	t.Run("localhost on the default port where nothing names one", func(t *testing.T) {
		...
		got := resolvedEnv(nil, nil, nil, nil, "", "", node)
	...
	t.Run("the browser's copy only where an app's bundle reads it", func(t *testing.T) {
		...
		got := resolvedEnv(nil, nil, dotfile, nil, "", "", envgate.Scope{Apps: []envgate.App{{Name: "api", Runtime: providerkit.RuntimeGo}}})
	...
	t.Run("the port the project names", func(t *testing.T) {
		...
		got := resolvedEnv(nil, nil, map[string]string{"PORT": "4321"}, nil, "", "", node)
...
func TestRunWritesTheBrowsersURLForTheAppItRunsIn(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got := resolvedEnv(nil, nil, nil, nil, "", "", targetScope(cfg, tc.cwd))
