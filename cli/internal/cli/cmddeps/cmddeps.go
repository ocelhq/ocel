...
type Deps struct {
	...
	BuildApp            func(ctx context.Context, cfg *projectconfig.Config, envByApp map[string]map[string]string, phase string, out io.Writer) error
	RequireImageBuilder func(ctx context.Context, rep runui.Reporter, cfg *projectconfig.Config) error
	BuildAppImages      func(ctx context.Context, cfg *projectconfig.Config, phase string, out io.Writer) (map[string]string, error)
