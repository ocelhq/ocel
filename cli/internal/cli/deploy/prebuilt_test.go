...
func TestCollectAndBuildManifest(t *testing.T) {
	t.Run("--prebuilt skips the build and carries the prebuilt tree's function", func(t *testing.T) {
		...
		manifest, err := collectAndBuildManifest(context.Background(), deps, cfg, noGate(cfg), true, s)
		if err != nil {
	...
	t.Run("without --prebuilt the build runs", func(t *testing.T) {
		...
		cfg := prebuiltConfig(root)
		if _, err := collectAndBuildManifest(context.Background(), deps, cfg, noGate(cfg), false, s); err != nil {
			t.Fatalf("collectAndBuildManifest: %v", err)
		}
		if !*ran {
	...
	t.Run("--prebuilt with no build output errors", func(t *testing.T) {
		...
		_, err := collectAndBuildManifest(context.Background(), deps, cfg, noGate(cfg), true, s)
		if err == nil {
	...
	t.Run("the client accessor is generated before the build", func(t *testing.T) {
		...
		s, _ := newBuildManifestSession(t)
		cfg := prebuiltConfig(root)
		if _, err := collectAndBuildManifest(context.Background(), deps, cfg, clientValueGate(t, cfg, "https://example.com"), false, s); err != nil {
			t.Fatalf("collectAndBuildManifest: %v", err)
		}
	...
	t.Run("--prebuilt refuses a stale client value", func(t *testing.T) {
		...
		_, err := collectAndBuildManifest(context.Background(), deps, cfg, gate, true, s)
		if err == nil {
	...
	t.Run("--prebuilt names an `ocel build` output for what it is", func(t *testing.T) {
		...
		s, _ := newBuildManifestSession(t)
		_, err := collectAndBuildManifest(context.Background(), deps, cfg, clientValueGate(t, cfg, "https://example.com"), true, s)
		if err == nil {
	...
	t.Run("--prebuilt proceeds when the client value is unchanged", func(t *testing.T) {
		...
		if _, err := collectAndBuildManifest(context.Background(), deps, cfg, gate, true, s); err != nil {
			t.Fatalf("collectAndBuildManifest: %v", err)
		}
	})
}
