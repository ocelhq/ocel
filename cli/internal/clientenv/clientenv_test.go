...
// worth anything if the framework's static replacement recognises it, and that
// recognition is a property of these exact bytes: a literal member expression
// per key and nothing indirect.
...
func TestGenerate_GivesEachAppItsOwnAccessor(t *testing.T) {
	...
	if got := read(t, filepath.Join(store, ".ocel", "env-client.ts")); !strings.Contains(got, "NEXT_PUBLIC_PUBLIC_SITE_URL") || strings.Contains(got, "ADMIN") {
		t.Errorf("storefront accessor = %q, want its own key only", got)
	}
	if got := read(t, filepath.Join(admin, ".ocel", "env-client.ts")); !strings.Contains(got, "NEXT_PUBLIC_PUBLIC_ADMIN_URL") || strings.Contains(got, "SITE") {
