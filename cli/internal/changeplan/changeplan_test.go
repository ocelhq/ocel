...
func TestRenderCountsAChildlessGroupAsOneItem(t *testing.T) {
	...
	var out bytes.Buffer
	changeplan.NewPrinter(&out, runui.Presentation{}).Render("Proposed changes to the preview bootstrap", &planv1.ChangePlan{
		Groups: []*planv1.ChangeGroup{
...
func TestPrintSeparatesTheDoomedFromTheKept(t *testing.T) {
	...
	var out bytes.Buffer
	changeplan.NewPrinter(&out, runui.Presentation{}).Print("This will permanently destroy production project \"shop\"", &planv1.ChangePlan{
		EdgeKind: "cloudflare",
		Groups: []*planv1.ChangeGroup{
