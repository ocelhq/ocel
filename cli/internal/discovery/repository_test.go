...
func TestRepositoryNamesTheDefaultDiscoveryDirectoryCentrally(t *testing.T) {
	...
	allowed := func(rel string) bool {
		if rel == "AGENTS.md" || rel == "CLAUDE.md" || rel == "www/content/docs/configuration.mdx" || rel == "console/github/src/server.ts" || strings.HasPrefix(rel, constants.DefaultDiscoveryDirName+"/") || strings.HasPrefix(rel, "packages/ocel/tests/fixtures/"+constants.DefaultDiscoveryDirName+"/") {
			return true
		}
